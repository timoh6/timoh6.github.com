questionData = [
    ...
    {
        question: 'You use a 104-bit, cryptographically strong, random number (hexadecimal encoded, for example) as your password on a web site which stores passwords as plain MD5 hashes: <span class="text-info">md5(password)</span>. Is it safe to assume your password will be safe if the user database leaks?',
        answers: [
            { id: 0, answer: 'Yes' },
    ...
    {
        question: 'You derive encryption keys from user&#039;s passwords using PBKDF2 algorithm. You implement PBKDF2 algorithm in userland PHP code because your PHP version lacks the support for hash_pbkdf2() function. Are there any (security-wise) disadvantages of having to implement the PBKDF2 algorithm in userland PHP code instead of being able to use the native PHP function hash_pbkdf2()?',
        answers: [
            { id: 0, answer: 'Yes, less efficient PHP userland implementation of PBKDF2 algorithm gives the attacker a greater change to raise the cracking/validation ratio' },
            { id: 1, answer: 'No, as long as the userland PBKDF2 implementation is a correct implementation of PBKDF2 algorithm, security-wise it does not make a difference' }

        ],
        currentAnswer: ko.observable(-2),
        correctAnswer: 0,
        comment: 'Userland PHP implementation of the PBKDF2 algorithm is less efficient than the native hash_pbkdf2() function which is implemented in C code, hence the worse &quot;cracking/validation ratio&quot;. In practice, this means the PHP userland implementation will not be able to stretch the password as much as the more efficient C implementation in a same amount of time.'
    },
    {
        question: 'Which of the following should be used to construct a "remember me" token?',
        answers: [
            { id: 0, answer: '20 bytes of random data from <span class="text-info">/dev/urandom</span>' },
            { id: 1, answer: '20 bytes of random data from <span class="text-info">/dev/urandom</span> stored via a hash (e.g. SHA-1)' },
            { id: 2, answer: 'A hash (e.g. SHA-1) of user&#039;s username, password and a salt concatenated' }

        ],
        currentAnswer: ko.observable(-2),
        correctAnswer: 1,
        comment: 'Constructing the token from user&#039;s credentials gains you nothing, but leaves an (unnecessary) attack vector open. If you construct the (long enough) token from strong random bytes, you can make sure the token is unguessable and will not leak any information about the user&#039;s credentials. If you also hash the random bytes, you can make sure an attacker who manages to steal those tokens from your database, will not be able to log in as the corresponding user, since she will not be able to obtain the original token.'
    },
    {
        question: 'You allow users to register user accounts on your web application on the internet. You hash users&#039; passwords using <span class="text-info">bcrypt</span> algorithm. Which of the following should be used as a guideline to adjust the <span class="text-info">bcrypt cost parameter</span> (to adjust the time needed to hash a password)?',
        answers: [
            { id: 0, answer: '20 milliseconds' },
            { id: 1, answer: '500 milliseconds' },
            { id: 2, answer: '1 second' }

        ],
        currentAnswer: ko.observable(-2),
        correctAnswer: 0,
        comment: 'You should not use too high work factor when dealing with interactive logins because password hashing becomes an easy target for DoS attacks (see <a href="http://timoh6.github.io/2013/11/26/Aggressive-password-stretching.html">here</a> for more information). Typically, password hashing time should be adjusted to take less than 100 ms. For example, as mentioned in the <a href="http://www.openwall.com/articles/PHP-Users-Passwords#dos">Solar Designer&#039;s article</a>: <em>For example, if a web application is not able to handle more than 50 requests per second for other reasons anyway, then having password authentication take 10 ms will not provide a significantly more attractive way of attack.</em>'
    }
];